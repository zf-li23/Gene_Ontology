...
degree = [0] * 105
maxd = 0
k1=1    #1 for tight, 0 for loose/very loose
k2=3    #3 for tight, 2.5 for loose, 2 for very loose
...
def dfs(u, dep, mp, group):
    ...
    for r in res:
        if (c[r[0]] > dep + 1 or c[r[0]] == 0) and r[1] >= dep - max(0, int((dep - k1) / k2)):
            ...
            temp.append(r[0])
            del mp[r[0]]
            thisgroup.append(r[0])
            dfs(r[0], dep + 1, mp, thisgroup)
            thisgroup.pop()
            mp[r[0]] = r[1]
    for v in renum:
        mp[v] -= 1
    ...
    for t in temp:
        c[t] = 0
    if dep == 2 and rememadd < len(add):
        maxaddsize = -1
        maxaddsizeid = -1
        for i in range(rememadd, len(add)):
            if len(add[i]) > maxaddsize:
                maxaddsize = len(add[i])
                maxaddsizeid = i
        add[rememadd], add[maxaddsizeid] = add[maxaddsizeid], add[rememadd]
        del add[rememadd + 1:]
        for i in range(len(add[rememadd])):
            if ec[add[rememadd][i]] == 1:
                c[add[rememadd][i]] = 2
    if ck == 0:
        thisgroup.sort()
...
def out():
    global spot, n
    with open("out3(tight).out", "w") as f:
        f.write(str(spot) + '\n')
    ...
    for i in range(n):
        ...
        for j in sp[i].s:
            ...
            if sp[v].ex == -1:
                continue
            ec[v] = 1
            print("    Marked ec[" + str(v) + "] = 1 for " + sp[v].name)
        add = []
